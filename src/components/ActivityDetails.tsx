...
export function ActivityDetails({ 
  ...
  const [eyfsHasChanges, setEyfsHasChanges] = useState(false);
  const [eyfsLastSaved, setEyfsLastSaved] = useState<Date | null>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  const imageInputRef = useRef<HTMLInputElement>(null);

  // Check if this is read-only mode (no onUpdate provided)
  ...
  useEffect(() => {
    ...
    setIsEditMode(isEditing && !isReadOnly);
  }, [activity, isEditing, isReadOnly]);
  ...
  const handleEyfsToggle = (eyfsStatement: string) => {
    // Allow EYFS editing even in read-only mode for lesson plans
    if (selectedEyfs.includes(eyfsStatement)) {
      ...
      if (activity.lessonNumber) {
        ...
        setEyfsLastSaved(new Date());
        // Show brief feedback
        setTimeout(() => setEyfsLastSaved(null), 3000);
      }
    } else {
    ...
    <div className="flex items-start space-x-3">
      ...
      <div>
        ...
        <p className="text-sm text-green-700 mt-1">
          ...
          <br />
          <span className="font-medium">✓ EYFS Standards are editable</span>
        </p>
      </div>
    </div>
  </div>
          ...
          <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
            ...
            {showEyfsSelector && activity.lessonNumber && (
              <div className="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
                ...
                {/* Auto-save notice for read-only mode */}
                {isReadOnly && (
                  <div className="mt-3 p-2 bg-green-50 border border-green-200 rounded text-xs text-green-700">
                    <div className="flex items-center space-x-1">
                      <Check className="h-3 w-3" />
                      <span>Changes are automatically saved to the lesson plan</span>
                    </div>
                  </div>
                )}
              </div>
